...
namespace SystemIO.Tests
{
    ...
    {
        //private const string _filePath = @"..\SystemIO.UI\output\StudentTest.txt";
        private const string _filePath = @"StudentTest.txt";
        private const string _originalData = @"StudentTestSeed.txt";

        [SetUp]
        public void Setup()
        {
            var file = Path.Combine(TestContext.CurrentContext.TestDirectory, @"StudentTest.txt");
            var seedFile = Path.Combine(TestContext.CurrentContext.TestDirectory, @"StudentTestseed.txt");
            if (File.Exists(file))
            {
                File.Delete(file);
            }

            File.Copy(seedFile, file);
        }
